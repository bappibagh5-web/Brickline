...
function LeadInput({ icon: Icon, value, onChange, placeholder, type = 'text' }) {
  return (
    <label className="relative block">
      <Icon className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#8a96b6]" />
      <input
        type={type}
        value={value}
        onChange={onChange}
        placeholder={placeholder}
        className="h-14 w-full rounded-lg border border-[#d4dbeb] bg-white pl-12 pr-4 text-[18px] text-[#2f3f66] placeholder:text-[#8a96b6] transition-all duration-150 focus:border-[#2f54eb] focus:outline-none"
      />
    </label>
  );
}
...
export default function LeadStep({
  ...
  const content = (
    <div className="space-y-4">
      ...
      <p className="pt-1 text-[13px] leading-6 text-[#6a7492]">
